...
export function VideoPlayer({ src, title, onClose }: VideoPlayerProps) {
      ...
      if (isHls) {
        try {
          ...
          if (Hls.isSupported()) {
            ...
            hls.loadSource(proxyUrl(videoSrc));
            hls.attachMedia(video);
          } else if (video.canPlayType("application/vnd.apple.mpegurl")) {
            video.src = proxyUrl(videoSrc);
  ...
  }, [src, useProxy, retryCount, setupPlayer, clearErrorTimer]);
      ...
      <div className="flex-1 relative flex items-center justify-center bg-black">
        ...
        {hasError && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 z-10 gap-4">
            ...
            {!useProxy && <p className="text-white/20 text-xs">Direct mode active</p>}
          </div>
        )}
      </div>
